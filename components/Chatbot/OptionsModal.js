...
const OptionsModal = ({ visible, onDismiss, onSubmit, steps, currentStep, state }) => {
      ...
      >
          ...
          <View style={{ position: 'relative', borderRadius: 12, overflow: 'hidden' }}>
                          ...
                          <View>
                            ...
                            {!!option?.field && (
                              <Text style={{ fontSize: 14, marginTop: 4, fontWeight: '500', opacity: .5 }}>
                                ...
                                || "Não informado."} */}
                                  { 
                                  Array.isArray(steps[option?.field]?.value) ?
                                  steps[option?.field]?.value?.join(" + ")
                                  : steps[option?.field]?.value
                                || "Não informado."}
                              </Text>
                            )}
                          </View>
                          {(currentStep?.type === "multiple" ? (
                            <Checkbox color={selecteds.find(selected => (selected === (option?.key || option?.value))) ? '#8257e6' : 'black'} 
                              size={24}
                              marked={!!selecteds.find(selected => (selected === (option?.key || option?.value)))}
                            />
                          ) : (
                            <Radiobox color={selecteds.find(selected => (selected === (option?.key || option?.value))) ? '#8257e6' : 'black'} 
                              size={24}
                              marked={!!selecteds.find(selected => (selected === (option?.key || option?.value)))}
                            />
                          ))}
            ...
            <TouchableOpacity style={[
              { flexGrow: 1, padding: 18, borderRadius: 20, backgroundColor: '#8257e6' },
