...
function formatCEP(cep) {
  if (!cep) return "";
  const numericCEP = cep.replace(/\D/g, ''); // Removes all non-numeric characters
  const formattedCEP = numericCEP.replace(/^(\d{5})(\d{3})$/, "$1-$2"); // Adds the mask "xxxxx-xxx"
  return formattedCEP;
}
...
const SimpleForm = () => {
    ...
    const theme = {
      light: {
        ...
        userFontColor: "#fafafa", userBubbleColor: "#8257e6",
    ...
    return (
      ...
      <Gradient colors={theme['light'].gradientColors} opacitys={theme['light'].gradientOpacitys} >
          ...
          submitButtonTextComponent={props => (
            <Feather name='send' style={{ opacity: 1 }} 
              size={24} 
              color={theme['light'].sendIconColor} 
            />
          )}
          ...
          steps={[
            ...
            },
            {
              ...
              trigger: 'cnpj-get-quest',
            },
            {
              id: 'cnpj-get-quest',
              message: 'Deseja carregar automaticamente informações sobre a empresa a partir do CNPJ?',
              trigger: 'cnpj-get-require',
            },
            {
              id: 'cnpj-get-require',
              ...
              options: [
                ...
                { key: "2", label: "Sim", trigger: 'cnpj-get', primary: true },
              ],
            },
            {
              id: 'cnpj-get',
            ...
            { 
              id: 'cnpj-error',
              message: 'Não foi possível buscar dados da empresa a partir do CNPJ.',
              trigger: 'handle-cnpj-quest',
            },
            {
              id: 'handle-cnpj-quest',
              message: 'Porfavor, informe os dados da empresa',
            ...
            { 
              id: 'cep-error',
              message: 'Não foi possível buscar dados de endereço a partir do CEP.',
              trigger: 'handle-andress-quest',
            },
            {
              id: 'handle-andress-quest',
              message: 'Porfavor, informe os dados de endereço',
              trigger: 'street-quest',
            },
            {
              id: 'andress-quest',
              message: 'Deseja carregar automaticamente informações de endereço a partir do CEP?',
              trigger: 'andress-require',
            },
            {
              id: 'andress-require',
              inputAttributes: { placeholder: "Escolha uma opção"},
              options: [
                { key: "1", label: "Não", trigger: 'street-quest' },
            ...
            // },
            {
              ...
              //   return !askwantchange ? 'update-andress-quest' : 'andress-update-fields-quest'
              // },
              trigger: 'update-andress-quest',
            },
            {
              id: 'update-andress-quest',
              message: 'Deseja alterar outros campos do endereço?',
              trigger: 'update-andress-require',
            },
            {
              id: 'update-andress-require',
              options: [
                { key: '1', label: "Não", trigger: '7' },
                { key: '2', label: "Sim", trigger: 'andress-quest', primary: true },
              ],
            },
            {
              id: 'andress-update-fields-quest',
              message: ({ steps, previousValue }) => `Deseja atualizar os campos de endereço com base no CEP ${formatCEP(previousValue ?? steps["cep"]?.value)} ?`,
              trigger: 'andress-update-fields-require',
            },
            {
              id: 'andress-update-fields-require',
              options: [
                { key: '1', label: "Não", trigger: "cep-quest" },
